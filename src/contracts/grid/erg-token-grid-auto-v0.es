{
  ...
  if (trading) {
    ...
    val validExchange = if (buying) {
      ...
      val limit = if (limits(BUY) > 0L) { limits(BUY) } else { SELF.value * price }; // token buy limit
      ...
      allOf(Coll(                        // should be true if:
        ...
        limit >= childBox.value * price, // 3. the token amount is within the limit
      ...
      val limit = if (limits(SELL) > 0L) { limits(SELL) } else { SELF.value * price }; // token sell limit
      ...
      allOf(Coll(                         // should be true if:
        ...
        limit >= childBox.tokens(T)._2,   // 3. the token amount is within the limit
